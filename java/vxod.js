...
/*        okno_vxoda_osnova.classList.toggle("okno_vxoda_osnova_itog");
    ...
    try{
    const SendingDataToTheServer = await LoginInformation("http://127.0.0.1:8000/login", {
        method: "POST",
        body: LoginUser
    })
    if(SendingDataToTheServer.success){
        okno_vxoda_osnova.classList.toggle("okno_vxoda_osnova_itog");
        okno_vxod_osnova_2.classList.toggle("okno_vxod_osnova_1")
        nadpis_yspex2.classList.toggle("nadpis_yspex2")
    } else{
        alert("Ошибка создания: ", SendingDataToTheServer.error)
    } 
    } catch (error) {
